...
const articleTranslations = {
  ru: {
    ...
    note: "Режим работы, вход и правила посещения могут меняться. Перед поездкой откройте официальный сайт места — мы добавляем ссылки на первоисточники.",
  ...
  en: {
    ...
    note: "Opening hours, admission and visitor rules can change. Check the official website before your visit — every card links to a primary source.",
...
function renderArticles(filter = "all") {
  ...
  grid.innerHTML = articles.filter((item) => filter === "all" || item.filter === filter).map((item) => `
    <article class="article-card">
      <div class="article-card-top"><span class="article-tag">${articleLanguage === "en" ? item.enTags : item.tags}</span><span class="article-number">${String(articles.indexOf(item) + 1).padStart(2, "0")}</span></div>
      <h2>${articleLanguage === "en" ? item.en : item.title}</h2>
      <p>${articleLanguage === "en" ? item.enText : item.text}</p>
      <a href="${item.url}" target="_blank" rel="noreferrer" class="article-link">${at("openOfficial")} <span aria-hidden="true">↗</span></a>
    </article>`).join("");
}
